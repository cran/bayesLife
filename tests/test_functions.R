...
test.estimate.mcmc <- function() {
	...
	cat('\n===> Test of estimating MCMC for extra areas OK.\n')
	# run prediction
...
test.existing.simulation <- function() {
	...
	cat('\n===> Test of retrieving projection results OK.\n')
}
...
test.e0trajectories <- function() {
	...
	unlink(filename)
	stopifnot(size > 0)
	cat('\n===> Test of plotting e0 trajectories OK.\n')
	t <- e0.trajectories.table(pred, 'Australia', pi=c(90, 80, 70))
	stopifnot(all(dim(t) == c(30, 7)))
	cat('\n===> Test of tabulating e0 trajectories OK.\n')
}
...
test.plot.map <- function() {
	...
	unlink(filename)
	stopifnot(size > 0)
	cat('\n===> Test of creating world maps OK.\n')
}
